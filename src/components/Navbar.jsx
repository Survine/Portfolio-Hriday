...
const Navbar = () => {
  ...
  //Scrolling logic of how to track the navbar
  useEffect(() => {
    const handleScroll = () => {
      setScrolled(window.scrollY > 10);
    };
    window.addEventListener("scroll", handleScroll); //Adds an event listener to the window object that listens for scroll events and calls the handleScroll function.
    return () => window.removeEventListener("scroll", handleScroll); //Makes the scroll event listener clean up after itself and not cause memory leaks.
  }, []);
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "py-2 bg-white/20 backdrop-blur-xl shadow-lg"
            ...
            <a href="/" aria-label="Home" className="flex items-center">
              <img
                ...
                className={`h-9 w-9 rounded-full object-cover border-2 transition-all duration-300 ${
                  scrolled ? "border-white/40" : "border-white/30"
                }`}
              />
